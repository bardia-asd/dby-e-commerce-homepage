...
export function ProductDetailPage() {
  const gallery = [images.blue, images.editorial, images.blue, images.editorial]
  ...
  return <main className="product-detail-page" dir="rtl">
    ...
    <div className="detail-shell">
      <div className="detail-breadcrumb"><a href="/">خانه</a><ChevronLeft size={15} /><a href="/">همه محصولات</a><ChevronLeft size={15} /><span>پالتوی پشم مرینو</span></div>
      <section className="detail-layout">
        <div className="detail-gallery"><div className="detail-main-image"><img src={gallery[activeImage]} alt="پالتوی پشم مرینو" /></div><div className="detail-thumbnails">{gallery.map((image, index) => <button className={index === activeImage ? 'active' : ''} onClick={() => setActiveImage(index)} key={`${image}-${index}`}><img src={image} alt={`نمای ${index + 1} محصول`} /></button>)}</div></div>
        <div className="detail-info"><span className="brand">DBY</span><h1>پالتوی پشم مرینو</h1><div className="detail-rating"><span>★★★★★</span><small>۱۲۴ نظر</small></div><div className="price-row"><strong>۴۸۵ دلار</strong><del>۶۸۰ دلار</del><b>٪۲۹ تخفیف</b></div><div className="option-block"><div className="option-label">رنگ <span>— {color}</span></div><div className="color-options"><button className={color === 'مشکی' ? 'selected black' : 'black'} onClick={() => setColor('مشکی')} aria-label="مشکی" /><button className={color === 'کرم' ? 'selected cream' : 'cream'} onClick={() => setColor('کرم')} aria-label="کرم" /><button className={color === 'سرمه‌ای' ? 'selected navy' : 'navy'} onClick={() => setColor('سرمه‌ای')} aria-label="سرمه‌ای" /></div></div><div className="option-block"><div className="option-label"><span>سایز</span><a href="#size">راهنمای سایز</a></div><div className="size-options">{['XS', 'S', 'M', 'L', 'XL'].map(item => <button className={size === item ? 'selected' : ''} onClick={() => { setSize(item); setWarning(false) }} key={item}>{item}</button>)}</div>{warning && <small className="size-warning">لطفاً یک سایز انتخاب کنید</small>}</div><div className="quantity-row"><b>تعداد</b><div><button onClick={() => setQuantity(Math.max(1, quantity - 1))} aria-label="کم کردن"><Minus size={14} /></button><span>{quantity}</span><button onClick={() => setQuantity(quantity + 1)} aria-label="زیاد کردن"><Plus size={14} /></button></div></div><div className="purchase-row"><button className="add-button" disabled={!selected} onClick={() => { if (!size) setWarning(true) }}>افزودن به سبد</button><button className={`favorite-button ${favorite ? 'selected' : ''}`} onClick={() => setFavorite(!favorite)} aria-label="افزودن به علاقه‌مندی‌ها"><Heart size={19} fill={favorite ? 'currentColor' : 'none'} /></button></div><button className="buy-button" disabled={!selected}>خرید فوری</button><div className="trust-mini"><span>♧ ارسال رایگان بالای ۱۵۰ دلار</span><span>↶ ۳۰ روز مهلت بازگشت</span><span>◇ ضمانت اصالت کالا</span></div><div className="detail-accordions">{panels.map(([id, title, text]) => <div className={`detail-accordion ${openPanel === id ? 'open' : ''}`} key={id}><button onClick={() => setOpenPanel(openPanel === id ? '' : id)}><b>{title}</b><ChevronDown size={16} /></button>{openPanel === id && <p>{text}</p>}</div>)}</div></div>
      </section>
