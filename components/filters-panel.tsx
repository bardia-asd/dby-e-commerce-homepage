...
export function FiltersContent({ onCategory }: { onCategory?: (value: string) => void }) {
  ...
  return <div className="filters-content" dir="rtl">
    ...
    <Section title="قیمت"><div className="price-range"><div className="range-track"><input aria-label="حداقل قیمت" type="range" min="50" max="200" value={min} onChange={(event) => setMin(Math.min(Number(event.target.value), max - 1))}/><input aria-label="حداکثر قیمت" type="range" min="50" max="200" value={max} onChange={(event) => setMax(Math.max(Number(event.target.value), min + 1))}/></div><div className="range-values"><span>${min}</span><span>${max}</span></div></div></Section>
...
export function FiltersPanel({ open, onOpenChange, onCategory }: { open: boolean; onOpenChange: (open: boolean) => void; onCategory?: (value: string) => void }) {
  return <><aside className="filters-sidebar"><FiltersContent onCategory={onCategory}/><button className="filters-apply" onClick={() => onOpenChange(false)}>اعمال فیلتر</button></aside>{open && <div className="filters-sheet-layer"><button className="filters-sheet-backdrop" aria-label="بستن فیلترها" onClick={() => onOpenChange(false)}/><aside className="filters-sheet"><header><h2>فیلترها</h2><button onClick={() => onOpenChange(false)} aria-label="بستن"><X size={24}/></button></header><FiltersContent onCategory={onCategory}/><button className="filters-apply" onClick={() => onOpenChange(false)}>اعمال فیلتر</button></aside></div>}</>
}
