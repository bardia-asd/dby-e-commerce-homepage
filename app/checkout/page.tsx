...
export default function CheckoutPage() {
  ...
  if (submitted) return <main className="checkout-page" dir="rtl"><section className="checkout-success"><div><Check size={34} /></div><span className="eyebrow orange">DBY ORDER</span><h1>سفارش شما با موفقیت ثبت شد!</h1><p>شماره سفارش شما: <strong>DBY-248631</strong></p><p>تحویل تقریبی: ۳ تا ۵ روز کاری</p><div><Link href="/" className="checkout-primary">بازگشت به فروشگاه</Link><Link href="/" className="checkout-secondary">مشاهده سفارش</Link></div></section><SiteFooter /><MobileBottomNav /></main>

  return <main className="checkout-page" dir="rtl"><header className="checkout-header"><Link href="/" className="logo">DBY</Link><MobileHeaderSearch /><div className="checkout-steps"><span className={!checkoutStarted ? "active" : "done"}>۱. سبد خرید</span><i /><span className={checkoutStarted ? "active" : ""}>۲. اطلاعات ارسال</span><i /><span>۳. پرداخت و ثبت سفارش</span></div><Link href="/" className="checkout-back"><ArrowRight size={16} /> بازگشت</Link></header>{!checkoutStarted ? <div className="checkout-cart-step"><section className="checkout-cart-card"><div className="checkout-step-heading"><span>۱</span><div><h1>سبد خرید شما</h1><p>محصولات انتخابی خود را بررسی کنید و سپس برای تکمیل سفارش ادامه دهید.</p></div></div>{items.length ? <div className="checkout-cart-items">{items.map((item) => <article className="checkout-cart-row" key={`${item.id}-${item.size}-${item.color}`}><img src={item.image} alt={item.name} /><div><small>DBY · {item.color}</small><h2>{item.name}</h2><p>سایز: {item.size} · تعداد: {item.quantity}</p></div><strong>{money(item.price * item.quantity)}</strong></article>)}</div> : <div className="checkout-empty checkout-cart-empty">سبد خرید شما خالی است.<Link href="/products">مشاهده محصولات</Link></div>}<div className="checkout-cart-actions"><Link href="/products" className="checkout-continue">ادامه خرید</Link><button type="button" className="checkout-next" disabled={!items.length} onClick={() => setCheckoutStarted(true)}>ادامه به اطلاعات ارسال <ArrowRight size={16} /></button></div></section><OrderSummary /></div> : <div className="checkout-layout"><OrderSummary /><form className="checkout-form" onSubmit={(event) => { event.preventDefault(); if (!accepted || !items.length) { setError(!items.length ? "سبد خرید شما خالی است." : "لطفاً قوانین و مقررات فروشگاه را تأیید کنید."); return } setError(""); setSubmitted(true) }}>
...
<SiteFooter /><MobileBottomNav /></main>
}
