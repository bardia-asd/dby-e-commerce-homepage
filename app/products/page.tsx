...
export default function ProductsPage() {
  ...
  return <main className="catalog-page" dir="rtl"><header className="catalog-header"><Link href="/" className="logo">DBY</Link><nav><Link href="/">خانه</Link><Link href="/products">محصولات</Link><Link href="/#collections">کالکشن‌ها</Link><Link href="/">درباره ما</Link></nav><div className="catalog-actions"><button onClick={openSearch} aria-label="جستجو"><Search size={18}/></button><Link href="/account" aria-label="حساب کاربری"><UserRound size={18}/></Link><button onClick={openCart} aria-label="سبد خرید"><ShoppingBag size={18}/>{cartCount > 0 && <b>{cartCount}</b>}</button><button className="catalog-menu" onClick={openMenu} aria-label="منو"><Menu size={19}/></button></div></header><section className="catalog-shell"><div className="catalog-breadcrumb"><Link href="/">خانه</Link><span>›</span><span>همه محصولات</span></div><h1>همه محصولات</h1><div className="catalog-toolbar"><button className="filter-trigger" onClick={() => setFilterOpen(true)}><SlidersHorizontal size={15}/> فیلترها</button><div className="category-tabs">{categories.map((item) => <button key={item} className={category === item ? 'active' : ''} onClick={() => setCategory(item)}>{item}</button>)}</div><span className="result-count">{visibleProducts.length} نتیجه</span><label className="sort-control"><span>مرتب‌سازی:</span><select value={sort} onChange={(event) => setSort(event.target.value)}><option>جدیدترین</option><option>قیمت: کم به زیاد</option><option>قیمت: زیاد به کم</option></select><ChevronDown size={14}/></label></div><div className="catalog-content-layout"><FiltersPanel open={filterOpen} onOpenChange={setFilterOpen} onCategory={(value) => setCategory(value === 'تی‌شرت' ? 'همه' : value)}/><div className="catalog-grid">{visibleProducts.map((product) => <ProductCard key={product.slug} product={product}/>)}</div></div></section><SiteFooter />{filterOpen && <div className="catalog-filter-layer"><button className="catalog-filter-backdrop" aria-label="بستن فیلترها" onClick={() => setFilterOpen(false)}/><aside className="catalog-filter-panel"><div><h2>فیلترها</h2><button onClick={() => setFilterOpen(false)} aria-label="بستن">×</button></div><label>دسته‌بندی</label>{categories.slice(1).map((item) => <button key={item} className={category === item ? 'selected' : ''} onClick={() => { setCategory(item); setFilterOpen(false) }}>{item}</button>)}<label>مرتب‌سازی</label>{['جدیدترین', 'قیمت: کم به زیاد', 'قیمت: زیاد به کم'].map((item) => <button key={item} className={sort === item ? 'selected' : ''} onClick={() => { setSort(item); setFilterOpen(false) }}>{item}</button>)}</aside></div>}</main>
}
